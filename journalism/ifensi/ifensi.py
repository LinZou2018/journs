...
def download(html, number, title):
    ...
    if not exist:
        return
    timeout = exist[0][0]
    pattern = re.compile("(<div class='p3'>)([\s\S]*?)(</div>)")
...
def getURL(html):
    pattern = re.compile('<div class="star_list">[\s\S]*?<div class="page2 clearfix">')
    ...
    for message in msg:
        ...
        # if rechecking(number, source_url="com.ifensi.news"):
        #     return True
        title = text[0][-2]
...
def starts():
    urls = ["http://news.ifensi.com/list-14-%s.html", "http://news.ifensi.com/list-15-%s.html",
            "http://news.ifensi.com/list-16-%s.html", "http://news.ifensi.com/list-17-%s.html"]
    for i in urls:
        n = 1
        while True:
            url = i % n
